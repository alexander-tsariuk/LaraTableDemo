...
class DateRangeFilter extends AbstractFilter
{
    ...
    {
        if (!empty($value['from'])) {
            $query->whereDate($this->attribute, '>=', date('Y-m-d', strtotime($value['from'])));
        }
        if (!empty($value['to'])) {
            $query->whereDate($this->attribute, '<=', date('Y-m-d', strtotime($value['to'])));
        }
    }
    ...
    {
        return view('laravel-table::' . config('laravel-table.ui') . '.date-range-filter', [
            'filter' => $this,
            'class' => $this->class(),
