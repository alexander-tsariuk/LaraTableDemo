...
class SummRangeFilter extends AbstractFilter
{
    public string $label;
    ...
    {
        if (!empty($value['from'])) {
            $query->where($this->attribute, '>=', intval($value['from']));
        }
        if (!empty($value['to'])) {
            $query->where($this->attribute, '<=', intval($value['to']));
        }
    }
    ...
    {
        return view('laravel-table::' . config('laravel-table.ui') . '.summ-range-filter', [
            'filter' => $this,
            'class' => $this->class(),
